...
- **Chain attack module** (`fuzzer/chain.rs`, #15) — the `chain` attack module is now implemented (previously advertised in `--attacks` but a no-op). `fuzzd audit --attacks chain --chains <PATH>` loads scripted attack chains (a JSON file or a directory of `*.json` files), executes each as an ordered tool-call sequence against a freshly spawned stdio session, records the run via the `SequenceObserver`, and feeds it to the sequence analyzer. A chain may declare an optional benign `baseline` run, executed in its own clean session, so the analyzer can diff baseline vs. adversarial to surface injected and diverged calls. Steps targeting a tool the server does not expose are skipped with a note rather than aborting the chain. Findings carry the chain id and (when set) the corpus `AttackRecord` id as provenance.
- **Mock peer-tool injection** (`fuzzer/peer.rs`, #16) — the `peer` attack module implements Paradigm 2 TPA detection. `fuzzd audit --attacks peer --cmd <server>` iterates over every TPA corpus record, injects it as a `MockPeerServer` alongside the real target, and runs two detection angles: (1) static scan of the peer tool's poisoned description using the full pattern library, and (2) a synthetic adversarial call sequence diffed against an empty baseline, which surfaces the peer tool as `UnexpectedToolSequence` — concrete proof the injected tool would appear in the live call log. `MockPeerTransport` is an in-process `Transport` that speaks the full MCP protocol (initialize → tools/list → tools/call) with no I/O, no subprocess, and no network. The module is opt-in (not included in the default module set) since it iterates the entire embedded TPA corpus per run.
...
- Total signals: 23 → 28 (FUZZD-024 through FUZZD-028 added).
